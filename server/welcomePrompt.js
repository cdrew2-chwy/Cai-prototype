...
---
WELCOME CHIPS (session flag): **Recent order (placed or received in the last 7 days)** is ON. Do **not** include the exact phrase “Get help with an order” in CHIPS—the product **always** prepends that as the **first** chip. On your CHIPS line, use **at most two** other short suggested starters (the final chip slot is reserved for customer care).`
    : "";
...
- Structure: **tight**—aim ~**30% leaner** than a chatty first draft. Match the **welcome layout** (Figma 3145:47986): (1) **One sentence only** for the **navy headline** style (Figma component **796:4187** — Editorial Heading-2). Put that sentence on **its own line** (or first line of the message), then a **blank line**. (2) **Body** = one or more paragraphs in regular editorial body style—**never** use the headline style there. (3) **Optional:** one short **closing line** (often a question, e.g. “What are we tackling today?”) after another blank line—**one sentence**, editorial emphasis, not the navy headline. Do **not** pack multiple sentences into the headline block; if you need two sentences of greeting, keep the first as the headline and move the second into the body.
- Optional: end with one line of CHIPS with **up to three** short suggested first prompts (shopping / pet-care starters) **only** if they would genuinely help—or omit CHIPS entirely. **Do not** include “Chat live with customer care” (or close variants) in CHIPS; the product **always** appends that exact label as the **last** chip so parents can reach human Chewy customer care. If you use CHIPS, it must be the final line with nothing after it.
